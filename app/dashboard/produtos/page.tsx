...
export default function ProdutosPage() {
  ...
  const columns: ColumnDef<Produto>[] = [
    ...
    { accessorKey: "unidadeDeMedida", header: "Unidade", cell: ({ row }) => <span className="uppercase">{row.original.unidadeDeMedida}</span> },
    {
      id: "actions",
      cell: ({ row }) => {
        ...
        return (
          <div className="text-right">
            <Button variant="ghost" size="icon" onClick={() => handleEdit(produto)}><IconPencil className="h-4 w-4" /></Button>
            <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive" onClick={() => handleDelete(produto.id!)}><IconTrash className="h-4 w-4" /></Button>
          </div>
        );
      },
    },
     ...
     <GenericForm schema={produtoSchema} onSubmit={onSubmit} formId="produto-form" form={form}>
        <div className="space-y-4">
          <FormField name="nome" control={form.control} render={({ field }) => (
            <FormItem><FormLabel>Nome do Produto</FormLabel><FormControl><Input placeholder="Ex: Picanha, Maminha" {...field} /></FormControl><FormMessage /></FormItem>
          )} />
          <FormField name="unidadeDeMedida" control={form.control} render={({ field }) => (
            <FormItem>
              ...
              <Select onValueChange={field.onChange} value={field.value} defaultValue={field.value}>
                ...
                <SelectContent>{unidadesDeMedida.map((unidade) => (<SelectItem key={unidade} value={unidade}>{unidade.toUpperCase()}</SelectItem>))}</SelectContent>
              </Select>
              <FormMessage />
            </FormItem>
          )} />
