...
export default function FornecedoresPage() {
    ...
    const handleFetch = async (type: 'cnpj' | 'cep') => {
        ...
        try {
            ...
            if (type === 'cnpj') {
                ...
                form.reset({
                    ...currentValues, razaoSocial: data.razao_social, contato: data.ddd_telefone_1 || '',
                    endereco: {
    ...
    const onSubmit = async (values: FornecedorFormValues) => {
        ...
        try {
            ...
            if (isEditing && id) {
                ...
                toast.success(`Fornecedor "${data.razaoSocial}" atualizado com sucesso!`, { id: toastId });
            } else {
                await addFornecedor(data);
                toast.success(`Fornecedor "${data.razaoSocial}" cadastrado com sucesso!`, { id: toastId });
            }
            resetForm();
        } catch (error: any) {
            toast.error("Ocorreu um erro ao salvar o fornecedor.", { id: toastId, description: error.message });
        }
    };
    ...
    return ( <CrudLayout formTitle={isEditing ? "Editar Fornecedor" : "Novo Fornecedor"} formContent={formContent} tableTitle="Fornecedores Cadastrados" tableContent={( <GenericTable columns={columns} data={fornecedores} filterPlaceholder="Filtrar por razão social..." filterColumnId="razaoSocial" /> )}/> );
}
