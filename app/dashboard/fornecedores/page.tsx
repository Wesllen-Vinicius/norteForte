...
export default function FornecedoresPage() {
    const { fornecedores, contasAPagar } = useDataStore();
    ...
    const handleOpenModal = useCallback((fornecedor: Fornecedor) => { setSelectedFornecedor(fornecedor); setIsModalOpen(true); }, []);
    const handleEdit = (fornecedor: Fornecedor) => { form.reset(fornecedor); setIsEditing(true); };
    const handleInactivate = async (id: string) => { if (confirm("Tem certeza?")) { try { await setFornecedorStatus(id, 'inativo'); toast.success("Fornecedor inativado!"); } catch { toast.error("Erro ao inativar."); } } };
                ...
                <DialogContent>
                    ...
                    {contasDoFornecedor.length > 0 ? (
                        <Table>
                            ...
                            <TableBody>{contasDoFornecedor.map((conta: any) => (<TableRow key={conta.id}><TableCell>{format(new Date(conta.dataVencimento), 'dd/MM/yyyy')}</TableCell><TableCell>R$ {conta.valor.toFixed(2)}</TableCell><TableCell>{conta.notaFiscal}</TableCell></TableRow>))}</TableBody>
                        </Table>
    ...
    return ( <> <ContasDoFornecedorModal />
        <CrudLayout formTitle={isEditing ? "Editar Fornecedor" : "Novo Fornecedor"} formContent={formContent} tableTitle="Fornecedores Cadastrados" tableContent={( <GenericTable columns={columns} data={fornecedores} filterPlaceholder="Filtrar por razão social..." filterColumnId="razaoSocial" renderSubComponent={renderSubComponent} /> )}/> </>
    );
}
